// login.js (highskills_erpnext override)
// Overrides frappe/templates/includes/login/login.js only to change the
// signup form submit handler (Personal/Company toggle + extra fields,
// posting to highskills_erpnext.api_signup.custom_sign_up instead of
// frappe.core.doctype.user.user.sign_up) and add the account-type toggle.
// Every other handler below is unchanged from core so login/forgot/OTP/etc.
...
login.bind_events = function () {
	...
	$(".account-type-options input[name=signup_account_type]").on("change", function () {
		login.update_signup_account_type();
	});
	...
	$(".form-signup").on("submit", function (event) {
		...
		var args = {};
		args.cmd = "highskills_erpnext.api_signup.custom_sign_up";
		args.account_type = account_type;
		args.first_name = frappe.utils.xss_sanitise(($("#signup_first_name").val() || "").trim());
		args.last_name = frappe.utils.xss_sanitise(($("#signup_last_name").val() || "").trim());
		args.email = ($("#signup_email").val() || "").trim();
		args.country = ($("#signup_country").val() || "").trim();
		args.password = $("#signup_password").val();
		args.confirm_password = $("#signup_confirm_password").val();
		args.redirect_to = frappe.utils.sanitise_redirect(frappe.utils.get_url_arg("redirect-to"));

		if (is_company) {
			args.job_title = frappe.utils.xss_sanitise(($("#signup_job_title").val() || "").trim());
			args.company_name = frappe.utils.xss_sanitise(($("#signup_company_name").val() || "").trim());
			args.company_id = frappe.utils.xss_sanitise(($("#signup_company_id").val() || "").trim());
		}

		var required = [args.first_name, args.last_name, args.email, args.country, args.password, args.confirm_password];
		if (is_company) {
			required = required.concat([args.job_title, args.company_name, args.company_id]);
		}

		if (required.some((value) => !value) || !validate_email(args.email)) {
			login.set_status({{ _("Please fill in all required fields with a valid email address") | tojson }}, 'red');
			return false;
		}

		if (args.password !== args.confirm_password) {
			login.set_status({{ _("Password and Confirm Password do not match") | tojson }}, 'red');
			return false;
		}
